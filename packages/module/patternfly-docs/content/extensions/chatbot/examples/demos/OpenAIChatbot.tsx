...
const openAIServerBaseUrl = 'http://127.0.0.1:8000';
// const openAIServerBaseUrl = 'http://127.0.0.1:11434';
const openAIToken = 'set-your-access-token';
...
const welcomePrompts = [
  {
    title: 'Models',
    message: 'You can use all models deployed on the OpenAI server. Pick a model in the above selector'
  },
  {
    title: 'Server',
    message: 'The current code expects an OpenAI server at ' + openAIServerBaseUrl
  }
];
...
export const OpenAIChatbotDemo: React.FunctionComponent = () => {
  ...
  // Fetch available models from the OpenAI server
  function getModels() {
    fetch(openAIServerBaseUrl + '/v1/models', {
      headers: {
      ...
      .then((data) => {
        ...
        for (const item of data.data) {
          const mp: ModelProps = { name: item.id, model: item.id };
          props.push(mp);
        }
  ...
  function getResponseFromLLM(mesgs: string[]): Promise<OpenAIChatResponse> {
    const mjson: OpenAIMessage[] = [];
    mesgs.forEach((msg) => {
      const o: OpenAIMessage = { role: 'user', content: msg };
      mjson.push(o);
    });
    const o: OpenAIMessage = {
      role: 'system',
      content: 'You are a helpful model. If you reply with source code, please format code snippets as Markdown'
    };
    mjson.push(o);
    ...
    const val = fetch(openAIServerBaseUrl + '/v1/chat/completions', {
      method: 'POST',
      headers: {
      ...
      .then((json) => {
        ...
        const oo: OpenAIChatResponse = Object.assign({}, json);
        console.log(JSON.stringify(oo.choices));
        return oo;
      });
    return val;
  }
  ...
  const onSelectModel = (
    ...
    // Trigger a "no-op" request which makes OpenAI load the model TODO true?
    getResponseFromLLM(['']);
  };
